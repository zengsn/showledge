...
public class SettingController {
	...
	public AjaxResult<Object> updateIntroduce(HttpSession session,
			String introduction, String site,Integer sex,String location,String academy) throws Exception {
		if (session.getAttribute("userIdInSession") != null) {
/*			int userId = (int) session.getAttribute("userIdInSession");
			
			userService.updateUserIntroduce(userId, introduction);
			userService.updateUserWeb(userId, site);
*/			
			if (sex == null) {
				sex = 2;
			}
			UserInfo se = new UserInfo(UserThreadLocal.getUser().getId(), sex, introduction, location, academy, site);
			userService.updateUserInfo(se);
