...
<body>
				...
				<div id="jsmind_nav">
								...
								<div class="modal-body">
									<form>
										<div class="form-group">
											...
											<input type="text" class="form-control" id="kmap-name" value="${kMap.kmapName}">
										</div>
										<div class="form-group">
											<label for="message-text" class="control-label">说明</label>
											<textarea class="form-control" id="kmap-describe">${kMap.kmapDescribe}</textarea>
										</div>
									</form>
								</div>
					...
					<div class="edit-list">
						...
						<ul class="edit-btn-list">
							...
							<li>
								<button onclick="delete_map();">删除地图</button>
							</li>
							<li>
	...
	<script>
		...
		function delete_map() {
			...
			var deleteKMapPath = kmapPath + "mindmap/deleteKMap";
			$.post(deleteKMapPath, {
	            "kmapId": kmapId
	        },
	        function(result) {
	            if (result && result['success']) {
					...
					location.href = kmapPath + "/user";
	            } else {
	            	$('.alert').removeClass('alert-success');
	        		$('.alert').addClass('alert-danger');
	        		$('.alert').text("删除失败,请刷新页面后重试~");
	            	$('.alert').slideDown();
					var Timer = setTimeout(function(){
					    $('.alert').slideUp();
					}, 2000);
		...
		$(document).ready(function() {
			...
			$("#finally-save").click(function(event) {
				...
				var updateKMapBasicSettingPath = kmapPath + "/mindmap/updateKMapBasicSetting";
				var updateKMapDataPath = kmapPath + "/mindmap/updateKMapData";
				$.post(updateKMapBasicSettingPath, {
		            "kmapId": kmapId,
		            "kmapName": kmapName,
		            ...
		            if (result && result['success']) {
				        ...
				        function(result) {
				            if (result && result['success']) {
								...
								location.href = kmapPath + "/mindmap/" + kmapId;
				            } else {
				            	$('.alert').removeClass('alert-success');
				        		$('.alert').addClass('alert-danger');
