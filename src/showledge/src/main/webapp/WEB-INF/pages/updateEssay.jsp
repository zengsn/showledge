...
<body id="edit-note-page">
    ...
    <div class="container main-content">
      <div class="row">
            ...
            <div class="col-md-10">
             <!-- 警告框 -->   
              ...
              <div class="editor-note-title">
                <input id="title" type="text" class="form-control" placeholder="请输入标题" value=${essay.essayTitle }>
              </div> 
              <!-- 笔记编辑区域 -->               
        ...
        <div class="col-md-2 send-back-btn">
          ...
          <a href="javascript:;"><button id="send-note-btn" class="btn btn-green">更新<span class="fa fa-caret-down fa-fw"></span></button></a>
          <div id="category-select">
            <form class="form-inline">
              ...
              <div class="form-group">
                 <select id="category-pick" class="selectpicker" data-live-search="true" data-live-search-style="begins" title="笔记分类" autocomplete="off">
                  <c:forEach items="${essaytype}" var="corpus" varStatus="status">
                  	<c:if test="${corpus.id ==  essay.type}">
	                  	<option name="${corpus.id }" selected="selected">${corpus.name }</option>
                  	</c:if>
                  	<c:if test="${corpus.id !=  essay.type}">
            ...
            <button id="updateNode" class="btn btn-info">确认更新</button> 
          </div>
        </div>
      </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        editor1.create();
        editor1.txt.html('${essay.essayContent }')
        
        $("#updateNode").click(function(event) {
  		...
  		//alert($("#category-pick").find("option:selected").attr("name"));
  		  $.post("writer/doUpdateNode.do",{essayid:'${essay.id}',title:title,content:content,type:$("#category-pick").find("option:selected").attr("name")},function(result){
