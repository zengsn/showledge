			...
			<div class="col-md-8 main">
				<div class="row">
										...
										<c:if test="${kmapList==null || fn:length(kmapList) <= 0}">
											<li>
												...
												<div class="mindmap-content">
													<a href="<%=path%>/newmindmap" class="mindmap-title">点击编写您的个人知识地图</a>
													<p class="mindmap-info">编写知识地图有助于你对整个结构的理解,梳理之后更加便于记忆,防止遗忘~</p>
												</div>
											</li>
										</c:if>
										<c:if test="${kmapList!=null && fn:length(kmapList) > 0}">
								...
								<div role="tabpanel" class="tab-pane fade" id="my-profile">
									<div class="profile-box">
										...
										<div class="profile-detail">
											<a href="resume/${user.id}" target="_blank" class="profile-title">${user.userName}的个人知识简历</a>
											<div class="profile-abstract">
												<span>个人信息：${user.userName}</span>
												<span>联系方式：${user.userPhone}</span>
												<span>联系邮箱：${user.userEmail}</span>
											</div>
											<a href="resume/${user.id}" target="_blank" id="more-profile-detail">点击查看完整简历 > </a>
										</div>
									</div>
								</div>
								<!-- my-profile end -->
					...
					<div class="col-md-3 aside">
						...
						<a class="function-btn" data-action="start-edit-intro" href="javascript:void(0)">
							<i class="glyphicon glyphicon-pencil"></i>
							编辑
						</a>
						<form class="profile-edit js-intro-form" data-type="json" accept-charset="UTF-8"
							data-remote="true" method="post">
							<textarea id="userIntroduce" name="userIntroduce"></textarea>
							<input type="button" value="保存" class="btn btn-hollow"
								onclick="userJS.updateUser('${user.userName}');">
							<a class="btn-cancel" data-action="cancel-edit-intro" href="javascript:void(null);">取消</a>
						</form>
